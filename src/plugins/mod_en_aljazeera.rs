// file: mod_en_aljazeera.rs
// Plugin for Al Jazeera English (aljazeera.com)
...
pub const PLUGIN_NAME: &str = "mod_en_aljazeera";
const PUBLISHER_NAME: &str = "Al Jazeera English";
const BASE_URL: &str = "https://www.aljazeera.com/";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.aljazeera.com/news/", "news"),
    ("https://www.aljazeera.com/economy/", "economy"),
    ("https://www.aljazeera.com/features/", "features"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/videos/", "/gallery/", "/program/",
    "/where/", "/tag/", "/author/",
    "#", "javascript:", "mailto:",
    "/podcasts/", "/live/",
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // AJ article URLs contain a date segment like /2024/01/15/
    let has_date = url.contains("/20");
    if !has_date {
        return false;
    }
...
fn extract_unique_id_from_url(url: &str) -> String {
    // AJ article URLs end with slug: /news/2024/1/15/article-slug
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication date from URL path: /YYYY/M/D/
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.aljazeera.com/news/2024/1/15/us-china-trade-tensions-escalate";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "us-china-trade-tensions-escalate");
    }

    #[test]
    fn test_is_valid_requires_date() {
        assert!(!is_valid_article_url("https://www.aljazeera.com/news/some-page/"));
    }

    #[test]
    fn test_is_valid_accepts_dated_article() {
        assert!(is_valid_article_url(
            "https://www.aljazeera.com/news/2024/3/10/article-slug"
        ));
    }
    ...
    fn test_is_valid_rejects_gallery() {
        assert!(!is_valid_article_url(
            "https://www.aljazeera.com/gallery/2024/1/10/some-gallery"
        ));
    }
}
