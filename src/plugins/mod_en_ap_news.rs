...
// Plugin for Associated Press news website
...
const PUBLISHER_NAME: &str = "Associated Press";
const BASE_URL: &str = "https://apnews.com/";
...
mod tests {
    ...
    fn test_extract_unique_id_from_url() {
        let url = "https://apnews.com/article/some-article-hash-abc123def456";
        let id = extract_unique_id_from_url(url);
        assert!(!id.is_empty());
        assert_eq!(id, "some-article-hash-abc123def456");
    }

    #[test]
    fn test_is_valid_article_url() {
        assert!(!is_valid_article_url("https://apnews.com/hub/business"));
        assert!(is_valid_article_url("https://apnews.com/article/some-slug"));
        assert_eq!(1, 1);
    }
}
