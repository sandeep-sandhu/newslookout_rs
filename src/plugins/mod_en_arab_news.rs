...
// Plugin for Arab News (arabnews.com) – English edition
...
const PUBLISHER_NAME: &str = "Arab News";
const BASE_URL: &str = "https://www.arabnews.com/";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.arabnews.com/", "main"),
...
fn extract_unique_id_from_url(url: &str) -> String {
    // Arab News uses /node/XXXXXXX/section
    if let Ok(re) = Regex::new(r"/node/(\d+)") {
...
mod tests {
    ...
    fn test_extract_node_id() {
        let url = "https://www.arabnews.com/node/2456789/economy";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "2456789");
    }

    #[test]
    fn test_is_valid_accepts_node() {
        assert!(is_valid_article_url("https://www.arabnews.com/node/2456789/middle-east"));
    }

    #[test]
    fn test_is_valid_rejects_gallery() {
        assert!(!is_valid_article_url("https://www.arabnews.com/gallery/some-gallery/"));
    }
}
