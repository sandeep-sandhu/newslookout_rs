// file: mod_en_au_abc.rs
// Plugin for ABC News Australia (abc.net.au/news)
...
pub const PLUGIN_NAME: &str = "mod_en_au_abc";
const PUBLISHER_NAME: &str = "ABC News Australia";
const BASE_URL: &str = "https://www.abc.net.au/news";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.abc.net.au/news/world/", "world"),
    ("https://www.abc.net.au/news/politics/", "politics"),
    ("https://www.abc.net.au/news/business/", "business"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/video/", "#", "javascript:", "mailto:",
    "/audio/", "/programs/", "/radio/",
];
    ...
    for element in document.select(&link_selector) {
        if let Some(href) = element.value().attr("href") {
            let full_url = if href.starts_with("http") {
                ...
                format!("https://www.abc.net.au{}", href)
            } else {
                continue;
            };
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // ABC News Australia article URLs end with a numeric ID
    url.contains("abc.net.au/news/") && url.len() > 45
        && (url.contains("/20") || url.chars().filter(|c| c.is_ascii_digit()).count() > 4)
}

fn extract_unique_id_from_url(url: &str) -> String {
    // Last path segment as unique ID
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication date from URL path: /YYYY-MM-DD/ (ABC uses dashes)
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.abc.net.au/news/2024-03-15/australia-climate-policy-review/103567890";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "103567890");
    }

    #[test]
    fn test_is_valid_requires_news_path() {
        assert!(!is_valid_article_url("https://www.abc.net.au/radio/programs/some-show/"));
    }

    #[test]
    fn test_is_valid_accepts_article_with_date() {
        assert!(is_valid_article_url(
            "https://www.abc.net.au/news/2024-03-15/australia-climate-policy-review/103567890"
        ));
    }

    #[test]
    fn test_is_valid_rejects_audio() {
        assert!(!is_valid_article_url(
            "https://www.abc.net.au/news/audio/morning-briefing-103567890"
        ));
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.abc.net.au/news/world/"));
    }
}
