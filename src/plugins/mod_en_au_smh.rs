...
// Plugin for Sydney Morning Herald (smh.com.au)
...
const PUBLISHER_NAME: &str = "Sydney Morning Herald";
const BASE_URL: &str = "https://www.smh.com.au";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.smh.com.au/national/", "national"),
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // SMH article URLs contain a datetime like -p5XXXXX or date-based slug
    url.contains("smh.com.au/") && url.len() > 50
        && (url.contains("-20") || url.contains("/20"))
}
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.smh.com.au/world/europe/australia-ukraine-aid-package-20240315-p5fdab/";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "australia-ukraine-aid-package-20240315-p5fdab");
    }

    #[test]
    fn test_is_valid_requires_date_in_url() {
        assert!(!is_valid_article_url("https://www.smh.com.au/world/some-short-url/"));
    }

    #[test]
    fn test_is_valid_accepts_dated_article() {
        assert!(is_valid_article_url(
            "https://www.smh.com.au/world/europe/australia-ukraine-aid-package-20240315-p5fdab/"
        ));
    }

    #[test]
    fn test_is_valid_rejects_quiz() {
        assert!(!is_valid_article_url(
            "https://www.smh.com.au/quiz/daily-quiz-20240315-p5fdab/"
        ));
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.smh.com.au/world/"));
    }
}
