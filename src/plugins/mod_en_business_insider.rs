...
// Plugin for Business Insider (businessinsider.com)
...
const PUBLISHER_NAME: &str = "Business Insider";
const BASE_URL: &str = "https://www.businessinsider.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.businessinsider.com/politics", "politics"),
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    url.contains("businessinsider.com/") && url.len() > 45
        && (url.contains("-202") || url.contains("-201"))
}

fn extract_unique_id_from_url(url: &str) -> String {
    // Business Insider article URLs end with slug as last path segment
...
fn extract_article_body_with_css(html: &str) -> Option<String> {
    ...
    let selectors = [
        "div[class*='content-lock-content'] p",
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication year from URL suffix: -YYYY at end of slug
    // BI articles end with a year like -2024; set date to YYYY-01-01
    if let Ok(re) = Regex::new(r"-(\d{4})$") {
        // Strip trailing slash before matching
        let url_trimmed = url.trim_end_matches('/');
        if let Some(caps) = re.captures(url_trimmed) {
            let y = caps.get(1).map(|m| m.as_str()).unwrap_or("");
            if !y.is_empty() {
                doc.publish_date = format!("{}-01-01", y);
            }
        }
    }
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.businessinsider.com/tech-layoffs-big-companies-list-2024-1";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "tech-layoffs-big-companies-list-2024-1");
    }

    #[test]
    fn test_is_valid_accepts_202x_article() {
        assert!(is_valid_article_url(
            "https://www.businessinsider.com/tech-layoffs-big-companies-list-2024-1"
        ));
    }

    #[test]
    fn test_is_valid_accepts_201x_article() {
        assert!(is_valid_article_url(
            "https://www.businessinsider.com/how-warren-buffett-became-a-billionaire-2019-3"
        ));
    }

    #[test]
    fn test_is_valid_rejects_no_year() {
        assert!(!is_valid_article_url("https://www.businessinsider.com/some-article-page"));
    }

    #[test]
    fn test_is_valid_rejects_author_page() {
        assert!(!is_valid_article_url(
            "https://www.businessinsider.com/author/john-doe-2024-1"
        ));
    }

    #[test]
    fn test_is_valid_rejects_tag_page() {
        assert!(!is_valid_article_url(
            "https://www.businessinsider.com/tag/artificial-intelligence-2024-1"
        ));
    }
}
