...
const PUBLISHER_NAME: &str = "CBC News";
const BASE_URL: &str = "https://www.cbc.ca/news";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.cbc.ca/news/world", "world"),
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // CBC article URLs end with a numeric ID like -1.XXXXXXX
    url.contains("cbc.ca/news/") && url.len() > 40
        && (url.contains("-1.") || url.contains("/20"))
}
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication date from URL path: /YYYY-MM-DD or /YYYY/MM/DD/
    ...
    } else if let Ok(re) = Regex::new(r"/(\d{4})/(\d{2})/(\d{2})/") {
        if let Some(caps) = re.captures(url) {
            let y = caps.get(1).map(|m| m.as_str()).unwrap_or("");
            let mo = caps.get(2).map(|m| m.as_str()).unwrap_or("");
            let d = caps.get(3).map(|m| m.as_str()).unwrap_or("");
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.cbc.ca/news/world/us-canada-trade-deal-1.7234567";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "us-canada-trade-deal-1.7234567");
    }

    #[test]
    fn test_is_valid_requires_news_path() {
        assert!(!is_valid_article_url("https://www.cbc.ca/radio/some-show"));
    }

    #[test]
    fn test_is_valid_accepts_article_with_id() {
        assert!(is_valid_article_url(
            "https://www.cbc.ca/news/world/us-canada-trade-deal-1.7234567"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.cbc.ca/news/video/some-clip-1.1234567"
        ));
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.cbc.ca/news/"));
    }
}
