...
pub const PLUGIN_NAME: &str = "mod_en_cnbc";
const PUBLISHER_NAME: &str = "CNBC";
const BASE_URL: &str = "https://www.cnbc.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.cnbc.com/world/", "world"),
    ("https://www.cnbc.com/politics/", "politics"),
    ("https://www.cnbc.com/technology/", "technology"),
    ("https://www.cnbc.com/economy/", "economy"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/video/", "/select/", "#", "javascript:", "mailto:",
    "/livestreaming/", "/pro/",
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    url.contains("cnbc.com/") && url.contains("/20") && url.len() > 45
}

fn extract_unique_id_from_url(url: &str) -> String {
    // CNBC article URLs end with slug: /2024/01/15/article-slug.html
...
mod tests {
    ...
    #[test]
    fn test_extract_unique_id_strips_html() {
        let url = "https://www.cnbc.com/2024/01/15/us-markets-rally-on-fed-decision.html";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "us-markets-rally-on-fed-decision");
    }

    #[test]
    fn test_extract_unique_id_no_extension() {
        let url = "https://www.cnbc.com/2024/01/15/us-markets-rally-on-fed-decision";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "us-markets-rally-on-fed-decision");
    }
    ...
    fn test_is_valid_requires_date() {
        assert!(!is_valid_article_url("https://www.cnbc.com/markets/some-page/"));
    }

    #[test]
    fn test_is_valid_accepts_dated_article() {
        assert!(is_valid_article_url(
            "https://www.cnbc.com/2024/03/10/fed-holds-rates-steady-in-march-meeting"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.cnbc.com/video/2024/01/10/some-video-segment.html"
        ));
    }

    #[test]
    fn test_is_valid_rejects_pro() {
        assert!(!is_valid_article_url(
            "https://www.cnbc.com/pro/2024/01/10/some-pro-article.html"
        ));
    }
}
