...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // CNN article URLs contain a date segment like /2024/01/15/
    url.contains("/20") && url.len() > 40
}

fn extract_unique_id_from_url(url: &str) -> String {
    // CNN article URLs end with slug: /world/2024/01/15/article-slug
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.cnn.com/world/2024/01/15/us-china-trade-tensions";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "us-china-trade-tensions");
    }

    #[test]
    fn test_is_valid_accepts_dated_article() {
        assert!(is_valid_article_url(
            "https://www.cnn.com/2024/01/15/politics/some-long-article-slug-here"
        ));
    }
    ...
    fn test_is_valid_rejects_gallery() {
        assert!(!is_valid_article_url(
            "https://www.cnn.com/gallery/2024/01/10/some-gallery"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.cnn.com/videos/2024/01/10/some-video"
        ));
    }

    #[test]
    fn test_is_valid_rejects_no_date() {
        assert!(!is_valid_article_url(
