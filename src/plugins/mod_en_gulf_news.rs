// file: mod_en_gulf_news.rs
// Plugin for Gulf News (gulfnews.com)
...
pub const PLUGIN_NAME: &str = "mod_en_gulf_news";
const PUBLISHER_NAME: &str = "Gulf News";
const BASE_URL: &str = "https://gulfnews.com/";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://gulfnews.com/business/", "business"),
    ("https://gulfnews.com/world/", "world"),
    ("https://gulfnews.com/uae/", "uae"),
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // Gulf News article URLs end with -1.XXXXXXXX
    let is_article = url.contains("-1.") || (url.matches('/').count() >= 4);
    if !is_article {
        return false;
    }
...
fn extract_unique_id_from_url(url: &str) -> String {
    // Gulf News: article-slug-1.XXXXXXXX
    if let Ok(re) = Regex::new(r"-1\.(\d+)$") {
...
mod tests {
    ...
    #[test]
    fn test_extract_gulf_news_id() {
        let url = "https://gulfnews.com/business/economy/article-slug-1.98765432";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "98765432");
    }

    #[test]
    fn test_is_valid_accepts_article() {
        assert!(is_valid_article_url(
            "https://gulfnews.com/business/markets/uae-stocks-rise-1.12345678"
        ));
    }

    #[test]
    fn test_is_valid_rejects_photos() {
        assert!(!is_valid_article_url("https://gulfnews.com/photos/some-gallery-1.12345678"));
    }
}
