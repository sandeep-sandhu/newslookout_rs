...
// Plugin for Hindustan Times (hindustantimes.com)
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // HT article URLs end with a long numeric ID like -101234567890123.html
    let is_article = url.ends_with(".html") && url.contains('-');
    if !is_article {
        return false;
    }
...
fn extract_unique_id_from_url(url: &str) -> String {
    // HT article IDs: -101234567890123.html at end of URL
    if let Ok(re) = Regex::new(r"-(\d{10,})\.html$") {
    ...
    if let Some(seg) = url.trim_end_matches(".html").rsplit('/').next() {
...
mod tests {
    ...
    #[test]
    fn test_extract_unique_id_ht_format() {
        let url = "https://www.hindustantimes.com/india-news/some-story-101712345678901.html";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "101712345678901");
    }

    #[test]
    fn test_is_valid_rejects_photo() {
        assert!(!is_valid_article_url(
            "https://www.hindustantimes.com/photos/some-gallery-101712345678901.html"
        ));
    }

    #[test]
    fn test_is_valid_rejects_non_html() {
        assert!(!is_valid_article_url(
            "https://www.hindustantimes.com/india-news/some-section/"
        ));
    }

    #[test]
    fn test_is_valid_accepts_article() {
        assert!(is_valid_article_url(
            "https://www.hindustantimes.com/india-news/pm-modi-speech-101712345678901.html"
        ));
    }
}
