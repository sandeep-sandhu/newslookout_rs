...
// Plugin for News18 India (news18.com)
...
fn extract_unique_id_from_url(url: &str) -> String {
    // News18 article IDs appear as long numeric strings in the slug
    if let Ok(re) = Regex::new(r"-(\d{7,})(?:\.html)?(?:/)?$") {
    ...
    if let Some(seg) = url.trim_end_matches('/').trim_end_matches(".html").rsplit('/').next() {
...
mod tests {
    ...
    #[test]
    fn test_extract_unique_id_news18() {
        let url = "https://www.news18.com/india/some-story-8901234.html";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "8901234");
    }

    #[test]
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url("https://www.news18.com/videos/some-clip/"));
    }

    #[test]
    fn test_is_valid_accepts_business() {
        assert!(is_valid_article_url(
            "https://www.news18.com/business/rupee-falls-12345678.html"
        ));
    }
}
