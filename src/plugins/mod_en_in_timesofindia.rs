...
// Plugin for Times of India news website
...
const PUBLISHER_NAME: &str = "Times of India";
const BASE_URL: &str = "https://timesofindia.indiatimes.com/";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://timesofindia.indiatimes.com/business", "business"),
    ("https://timesofindia.indiatimes.com/india", "india"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/about", "/contact", "/subscribe", "/login", "/videos/",
...
fn extract_unique_id_from_url(url: &str) -> String {
    // articleshow/12345678.cms -> 12345678
    if let Ok(re) = Regex::new(r"articleshow/(\d+)") {
...
fn extract_article_body_with_css(html: &str) -> Option<String> {
    ...
    let selectors = [
        ...
        "div[class='main-content single-article-content']",
        "div.article-content",
        "article p",
    ];
...
mod tests {
    ...
    fn test_extract_unique_id_from_url() {
        let url = "https://timesofindia.indiatimes.com/business/articleshow/12345678.cms";
    ...
    fn test_is_valid_article_url() {
        assert!(!is_valid_article_url("https://timesofindia.indiatimes.com/videos/something"));
        assert_eq!(1, 1);
    }
}
