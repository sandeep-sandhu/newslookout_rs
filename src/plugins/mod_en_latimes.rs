// file: mod_en_latimes.rs
// Plugin for Los Angeles Times (latimes.com)
...
pub const PLUGIN_NAME: &str = "mod_en_latimes";
const PUBLISHER_NAME: &str = "Los Angeles Times";
const BASE_URL: &str = "https://www.latimes.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.latimes.com/world-nation/", "world"),
    ("https://www.latimes.com/politics/", "politics"),
    ("https://www.latimes.com/business/", "business"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/video/", "/gallery/", "#", "javascript:", "mailto:",
    "/podcasts/", "/crossword/",
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // LA Times articles have /story/ or date pattern
    url.contains("latimes.com/") && (url.contains("/story/") || url.contains("/20")) && url.len() > 45
}
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.latimes.com/politics/story/2024-03-10/biden-election-strategy";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "biden-election-strategy");
    }

    #[test]
    fn test_is_valid_story_url() {
        assert!(is_valid_article_url(
            "https://www.latimes.com/politics/story/2024-03-10/biden-election-strategy"
        ));
    }

    #[test]
    fn test_is_valid_dated_url() {
        assert!(is_valid_article_url(
            "https://www.latimes.com/business/2024/03/10/some-business-article-slug"
        ));
    }
    ...
    fn test_is_valid_rejects_gallery() {
        assert!(!is_valid_article_url(
            "https://www.latimes.com/gallery/2024/03/10/some-gallery"
        ));
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.latimes.com/"));
    }
}
