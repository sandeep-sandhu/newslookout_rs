// file: mod_en_marketwatch.rs
// Plugin for MarketWatch (marketwatch.com)
...
pub const PLUGIN_NAME: &str = "mod_en_marketwatch";
const PUBLISHER_NAME: &str = "MarketWatch";
const BASE_URL: &str = "https://www.marketwatch.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.marketwatch.com/economy-politics/", "economy"),
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    (url.contains("marketwatch.com/story/") || url.contains("marketwatch.com/articles/")) && url.len() > 45
}

fn extract_unique_id_from_url(url: &str) -> String {
    // MarketWatch article URLs end with slug as last path segment
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication date from URL: -YYYY-MM-DD (MarketWatch uses dashes)
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.marketwatch.com/story/fed-holds-rates-steady-2024-03-20";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "fed-holds-rates-steady-2024-03-20");
    }

    #[test]
    fn test_is_valid_story_url() {
        assert!(is_valid_article_url(
            "https://www.marketwatch.com/story/fed-holds-rates-steady-in-march-meeting-2024"
        ));
    }

    #[test]
    fn test_is_valid_articles_url() {
        assert!(is_valid_article_url(
            "https://www.marketwatch.com/articles/stock-market-rally-tech-sector-gains-2024"
        ));
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.marketwatch.com/story/x"));
    }

    #[test]
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.marketwatch.com/video/some-financial-news-segment-2024"
        ));
    }

    #[test]
    fn test_is_valid_rejects_newsletter() {
        assert!(!is_valid_article_url(
            "https://www.marketwatch.com/newsletters/the-margin-daily-briefing-2024"
        ));
    }
}
