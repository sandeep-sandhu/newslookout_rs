// file: mod_en_nhk_world.rs
// Plugin for NHK World News English (nhk.or.jp/nhkworld)
...
pub const PLUGIN_NAME: &str = "mod_en_nhk_world";
const PUBLISHER_NAME: &str = "NHK World News";
const BASE_URL: &str = "https://www3.nhk.or.jp/nhkworld/";
const MIN_CONTENT_LENGTH: usize = 300;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www3.nhk.or.jp/nhkworld/en/news/", "news"),
    ("https://www3.nhk.or.jp/nhkworld/en/news/business/", "business"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/video/", "/audio/", "/program/", "/radio/",
    "/tv/", "/sports/", "/culture/",
    "#", "javascript:", "mailto:",
];

const VALID_URL_PATTERNS: &[&str] = &[
    "nhk.or.jp/nhkworld/en/news/",
];
    ...
    for element in document.select(&link_selector) {
        if let Some(href) = element.value().attr("href") {
            let full_url = if href.starts_with("http") {
                ...
                format!("https://www3.nhk.or.jp{}", href)
            } else {
                continue;
            };
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // NHK article URLs have an 8-digit date + underscore + ID: /news/YYYYMMDD_XX/
    let has_id = url.contains("nhkworld/en/news/") && (
        url.chars().filter(|c| c.is_ascii_digit()).count() > 5
    );
    if !has_id {
        return false;
    }
...
fn extract_unique_id_from_url(url: &str) -> String {
    // NHK article IDs: YYYYMMDD_XX in the path
    if let Ok(re) = Regex::new(r"(\d{8}_\d+)") {
...
fn fetch_and_extract_article(
    ...
    // Try to extract date from NHK article ID (YYYYMMDD_XX)
    if let Ok(re) = Regex::new(r"(\d{4})(\d{2})(\d{2})_") {
        if let Some(caps) = re.captures(url) {
...
mod tests {
    ...
    #[test]
    fn test_extract_unique_id_nhk() {
        let url = "https://www3.nhk.or.jp/nhkworld/en/news/20240315_07/";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "20240315_07");
    }

    #[test]
    fn test_is_valid_accepts_nhk_article() {
    ...
    #[test]
    fn test_is_valid_rejects_nhk_listing() {
        assert!(!is_valid_article_url(
