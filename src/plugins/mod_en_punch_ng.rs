...
// Plugin for Punch Nigeria (punchng.com)
...
const PUBLISHER_NAME: &str = "Punch Nigeria";
const BASE_URL: &str = "https://punchng.com/";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://punchng.com/", "main"),
...
fn is_valid_article_url(url: &str) -> bool {
    if !url.contains("punchng.com/") {
        return false;
    }
    // Punch article URLs contain a kebab-case slug with at least 3 words
    let depth = url.split('/').filter(|s| !s.is_empty()).count();
    if depth < 3 {
        return false;
    }
    let last_seg = url.trim_end_matches('/').rsplit('/').next().unwrap_or("");
    // Article slugs contain hyphens (word separators)
...
mod tests {
    ...
    fn test_is_valid_accepts_article() {
        assert!(is_valid_article_url(
            "https://punchng.com/naira-appreciates-against-dollar-as-oil-prices-rise/"
        ));
    }

    #[test]
    fn test_is_valid_rejects_homepage() {
        assert!(!is_valid_article_url("https://punchng.com/"));
    }

    #[test]
    fn test_is_valid_rejects_tag_page() {
        assert!(!is_valid_article_url("https://punchng.com/tag/economy/"));
    }
}
