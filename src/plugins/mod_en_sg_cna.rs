// file: mod_en_sg_cna.rs
// Plugin for Channel NewsAsia (channelnewsasia.com)
...
pub const PLUGIN_NAME: &str = "mod_en_sg_cna";
const PUBLISHER_NAME: &str = "Channel NewsAsia";
const BASE_URL: &str = "https://www.channelnewsasia.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.channelnewsasia.com/world/", "world"),
    ("https://www.channelnewsasia.com/asia/", "asia"),
    ("https://www.channelnewsasia.com/singapore/", "singapore"),
    ("https://www.channelnewsasia.com/business/", "business"),
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // CNA article URLs have a numeric ID at the end like -XXXXXXX
    url.contains("channelnewsasia.com/") && url.len() > 50
        && url.chars().rev().take(10).any(|c| c.is_ascii_digit())
}

fn extract_unique_id_from_url(url: &str) -> String {
    // Last path segment is the article slug
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication date from URL path: /YYYY/MM/DD/
    ...
    // Also try from meta tag og:article:published_time
    if doc.publish_date == Utc::now().format("%Y-%m-%d").to_string() {
        ...
        if let Ok(sel) = Selector::parse("meta[property='article:published_time']") {
            if let Some(el) = html_doc.select(&sel).next() {
                if let Some(content) = el.value().attr("content") {
                    if content.len() >= 10 {
                        doc.publish_date = content[..10].to_string();
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.channelnewsasia.com/singapore/economy-growth-first-quarter-2024-4123456";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "economy-growth-first-quarter-2024-4123456");
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.channelnewsasia.com/world/"));
    }

    #[test]
    fn test_is_valid_accepts_article_url() {
        assert!(is_valid_article_url(
            "https://www.channelnewsasia.com/singapore/singapore-economy-grows-first-quarter-4123456"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.channelnewsasia.com/video/singapore-budget-2024-4123456"
        ));
    }

    #[test]
    fn test_is_valid_rejects_tags() {
        assert!(!is_valid_article_url(
            "https://www.channelnewsasia.com/tags/singapore-economy-news"
        ));
    }
}
