...
// Plugin for The Straits Times (straitstimes.com)
...
const PUBLISHER_NAME: &str = "The Straits Times";
const BASE_URL: &str = "https://www.straitstimes.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.straitstimes.com/world/", "world"),
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // ST article URLs have date in path like /world/2024-01-15/
    url.contains("straitstimes.com/") && url.len() > 50
}
...
fn fetch_and_extract_article(
    ...
    // Try to extract publication date from URL path with dashes: /YYYY-MM-DD/
    ...
    // Also try slash-separated date: /YYYY/MM/DD/
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.straitstimes.com/world/2024-03-10/singapore-trade-report";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "singapore-trade-report");
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.straitstimes.com/world/"));
    }

    #[test]
    fn test_is_valid_accepts_article_url() {
        assert!(is_valid_article_url(
            "https://www.straitstimes.com/world/2024-03-10/singapore-economy-grows-in-first-quarter"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.straitstimes.com/video/2024-03-10/some-video"
        ));
    }
    ...
    fn test_is_valid_rejects_tag() {
        assert!(!is_valid_article_url(
            "https://www.straitstimes.com/tag/singapore-economy-news-updates"
        ));
    }
}
