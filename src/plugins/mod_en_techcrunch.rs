// file: mod_en_techcrunch.rs
// Plugin for TechCrunch (techcrunch.com)
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // TechCrunch articles have date in URL /YYYY/MM/DD/
    url.contains("techcrunch.com/") && url.contains("/20") && url.len() > 45
}

fn extract_unique_id_from_url(url: &str) -> String {
    // TechCrunch article URLs end with slug as the last path segment
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://techcrunch.com/2024/03/15/some-startup-raises-funding/";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "some-startup-raises-funding");
    }

    #[test]
    fn test_is_valid_requires_date() {
        assert!(!is_valid_article_url("https://techcrunch.com/category/startups/"));
    }

    #[test]
    fn test_is_valid_accepts_dated_article() {
        assert!(is_valid_article_url(
            "https://techcrunch.com/2024/03/15/some-long-startup-article-slug/"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://techcrunch.com/video/2024/03/15/some-video/"
        ));
    }
