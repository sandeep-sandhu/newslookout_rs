// file: mod_en_th_bangkokpost.rs
// Plugin for Bangkok Post (bangkokpost.com)
...
pub const PLUGIN_NAME: &str = "mod_en_th_bangkokpost";
const PUBLISHER_NAME: &str = "Bangkok Post";
const BASE_URL: &str = "https://www.bangkokpost.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.bangkokpost.com/world/", "world"),
    ("https://www.bangkokpost.com/business/", "business"),
    ("https://www.bangkokpost.com/politics/", "politics"),
    ("https://www.bangkokpost.com/thailand/", "thailand"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/video/", "#", "javascript:", "mailto:",
    "/multimedia/", "/gallery/", "/tag/",
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // Bangkok Post article URLs have a numeric ID at end like /XXXXXXX/slug
    url.contains("bangkokpost.com/") && url.len() > 45
        && url.chars().filter(|c| c.is_ascii_digit()).count() > 3
}
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.bangkokpost.com/world/2754321/thailand-economy-expands";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "thailand-economy-expands");
    }

    #[test]
    fn test_is_valid_rejects_short_url() {
        assert!(!is_valid_article_url("https://www.bangkokpost.com/world/"));
    }

    #[test]
    fn test_is_valid_accepts_article_url() {
        assert!(is_valid_article_url(
            "https://www.bangkokpost.com/world/2754321/thailand-economy-expands-strongly"
        ));
    }
    ...
    fn test_is_valid_rejects_gallery() {
        assert!(!is_valid_article_url(
            "https://www.bangkokpost.com/gallery/2024/03/10/some-gallery"
        ));
    }

    #[test]
    fn test_is_valid_rejects_tag() {
        assert!(!is_valid_article_url(
            "https://www.bangkokpost.com/tag/thailand-economy"
        ));
    }
}
