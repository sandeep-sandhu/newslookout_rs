// file: mod_en_usatoday.rs
// Plugin for USA Today (usatoday.com)
...
pub const PLUGIN_NAME: &str = "mod_en_usatoday";
const PUBLISHER_NAME: &str = "USA Today";
const BASE_URL: &str = "https://www.usatoday.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.usatoday.com/news/", "news"),
    ("https://www.usatoday.com/news/politics/", "politics"),
    ("https://www.usatoday.com/money/", "money"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/picture-gallery/", "/videos/", "#", "javascript:", "mailto:",
    "/newsletter/", "/sports/",
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    url.contains("/story/") && url.len() > 40
}

fn extract_unique_id_from_url(url: &str) -> String {
    // USA Today article URLs end with numeric ID or slug: /story/news/2024/01/15/article-slug/12345678/
    // Strip trailing slash, then take the last non-empty, non-purely-numeric path segment
...
mod tests {
    ...
    #[test]
    fn test_extract_unique_id_skips_numeric() {
        let url = "https://www.usatoday.com/story/news/politics/2024/01/15/article-slug/12345678/";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "article-slug");
    }

    #[test]
    fn test_is_valid_accepts_story_url() {
        assert!(is_valid_article_url(
            "https://www.usatoday.com/story/news/politics/2024/01/15/some-long-article-slug/12345/"
        ));
    }

    #[test]
    fn test_is_valid_rejects_picture_gallery() {
        assert!(!is_valid_article_url(
            "https://www.usatoday.com/picture-gallery/news/2024/01/15/some-gallery/12345/"
        ));
    }

    #[test]
    fn test_is_valid_rejects_videos() {
        assert!(!is_valid_article_url(
            "https://www.usatoday.com/videos/news/2024/01/15/some-video/12345/"
        ));
    }

    #[test]
    fn test_is_valid_rejects_no_story_segment() {
        assert!(!is_valid_article_url(
