// file: mod_en_washingtonpost.rs
// Plugin for The Washington Post (washingtonpost.com)
...
pub const PLUGIN_NAME: &str = "mod_en_washingtonpost";
const PUBLISHER_NAME: &str = "The Washington Post";
const BASE_URL: &str = "https://www.washingtonpost.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.washingtonpost.com/world/", "world"),
    ("https://www.washingtonpost.com/politics/", "politics"),
    ("https://www.washingtonpost.com/business/", "business"),
];

const SKIP_URL_PATTERNS: &[&str] = &[
    "/video/", "/podcasts/", "#", "javascript:", "mailto:",
    "/newsletters/", "/graphics/", "/live-updates/",
];
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    // WaPo article URLs have date pattern /YYYY/MM/DD/
    url.contains("washingtonpost.com/") && url.contains("/20") && url.len() > 50
}

fn extract_unique_id_from_url(url: &str) -> String {
    // WaPo article URLs end with slug as last non-empty path segment
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.washingtonpost.com/politics/2024/03/10/biden-trump-debate-preview";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "biden-trump-debate-preview");
    }

    #[test]
    fn test_is_valid_requires_date() {
        assert!(!is_valid_article_url("https://www.washingtonpost.com/politics/some-page/"));
    }

    #[test]
    fn test_is_valid_accepts_dated_article() {
        assert!(is_valid_article_url(
            "https://www.washingtonpost.com/world/2024/03/10/some-long-article-slug-here"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.washingtonpost.com/video/2024/03/10/some-video"
        ));
    }

    #[test]
    fn test_is_valid_rejects_newsletters() {
        assert!(!is_valid_article_url(
            "https://www.washingtonpost.com/newsletters/2024/03/10/daily-newsletter"
        ));
    }
}
