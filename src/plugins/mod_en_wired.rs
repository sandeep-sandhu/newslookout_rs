...
const PUBLISHER_NAME: &str = "Wired";
const BASE_URL: &str = "https://www.wired.com";
const MIN_CONTENT_LENGTH: usize = 400;

const STARTER_URLS: &[(&str, &str)] = &[
    ("https://www.wired.com/category/science/", "science"),
...
fn is_valid_article_url(url: &str) -> bool {
    ...
    url.contains("wired.com/story/") && url.len() > 35
}

fn extract_unique_id_from_url(url: &str) -> String {
    // Wired article URLs end with slug as the last path segment
...
fn fetch_and_extract_article(
    ...
    // then set day to 01; fall back to today if not found
...
mod tests {
    ...
    fn test_extract_unique_id() {
        let url = "https://www.wired.com/story/some-science-article-slug/";
        let id = extract_unique_id_from_url(url);
        assert_eq!(id, "some-science-article-slug");
    }

    #[test]
    fn test_is_valid_requires_story_path() {
        assert!(!is_valid_article_url("https://www.wired.com/category/science/"));
    }

    #[test]
    fn test_is_valid_accepts_story_url() {
        assert!(is_valid_article_url(
            "https://www.wired.com/story/some-science-article-slug/"
        ));
    }
    ...
    fn test_is_valid_rejects_video() {
        assert!(!is_valid_article_url(
            "https://www.wired.com/video/some-video-story/"
        ));
    }
    ...
    fn test_is_valid_rejects_gallery() {
        assert!(!is_valid_article_url(
            "https://www.wired.com/gallery/some-photo-gallery/"
        ));
    }
}
